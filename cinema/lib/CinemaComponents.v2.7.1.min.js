...
(function() {
	 ...
	 */
	CINEMA_COMPONENTS.Database = function(directory, callback, errorCallback, filter, db_query) {
		...
		this.dsi_db = null;
		this.dsi_db_schema = "";
		this.queries = db_query;
		this.current_selected_query = db_query[0];
		// console.log("this.queries", this.queries)
		this.current_query_result = [];
	...
	// Edited by Mengjiao 
	CINEMA_COMPONENTS.Database.prototype.addQuery = function(queryText){
		this.queries.push(queryText);
		console.log("queries:", this.queries);
	}
	CINEMA_COMPONENTS.Database.prototype.setCurrentQueryText = function(queryText){
		this.current_selected_query = queryText;
	}
	CINEMA_COMPONENTS.Database.prototype.getQueryResults = function(queryText){
		if(this.current_query_result.length !== 0){
			this.current_query_result = [];

		}
		// console.log("current query", queryText)
		// find the current query results 
		const stmt = this.dsi_db.prepare(queryText);
		while(stmt.step()) { 
            const row = stmt.getAsObject();
            const temp = JSON.parse(JSON.stringify(row));
            // console.log("query", temp)
            this.current_query_result.push(temp);
        }
		this.data = this.current_query_result;
		// console.log("query results", this.data)
	}
	CINEMA_COMPONENTS.Database.prototype.getDatabaseSchema = function(){
		const stmt_read = this.dsi_db.prepare("SELECT name FROM sqlite_master WHERE type='table'");
		while(stmt_read.step()){
			const temp = stmt_read.getAsObject();
			const temp_row = JSON.parse(JSON.stringify(temp));
			const tablename = temp_row['name']
			const query_read = this.dsi_db.prepare(`SELECT sql FROM sqlite_master WHERE name='${tablename}'`);
			while(query_read.step()){
				const temp = query_read.getAsObject();
				const temp_row = JSON.parse(JSON.stringify(temp));
				this.dsi_db_schema = temp_row.sql;
			}
		}
	}

	CINEMA_COMPONENTS.Database.prototype.loadDSIDatabaseFunc = function(db){
		var self = this;
		this.dsi_db = db;
		const stmt_read = self.dsi_db.prepare("SELECT * FROM cdb");
		while(stmt_read.step()){
			const temp = stmt_read.getAsObject();
			const temp_row = JSON.parse(JSON.stringify(temp));
			// console.log("string",temp_row);
			self.dsi_data.push(temp_row)
		}
		// find schema
...
(function() {
	 ...
	 * DATABASEVIEW
	 *
	 * The DatabaseView Component for the CINEMA_COMPONENTS library.
	 * Contains the constructor for the DatabaseView Component
	 * Which displays schema and information in a database.
	 *
	 * @exports CINEMA_COMPONENTS
	 *
	 * @author Mengjiao Han
	...
	if(!CINEMA_COMPONENTS.COMPONENT_INCLUDED)
		throw new Error("CINEMA_COMPONENTS DatabaseView module requires that Component" +
			" module be included. Please make sure that Component module" +
			" is included BEFORE DatabaseView module");

	//Require that d3 be included
	if(!window.d3) {
		throw new Error("CINEMA_COMPONENTS DatabaseView module requires that" +
			" d3 be included (at least d3v4). Please make sure that d3 is included BEFORE the" +
			" the DatabaseView module");
	}

	/** @type {boolean} - Flag to indicate that the ImageSpread module has been included */
	CINEMA_COMPONENTS.DATABASE_VIEW_INCLUDED = true;
	 ...
	 */
	CINEMA_COMPONENTS.DatabaseView = function(parent, database, directory) {
		var self = this;
		 ...
		 ***************************************/
		this.db = database;
		this.er_diagram = null;
		this.diagram_path = directory + "/diagram.png"
		...
		d3.select(this.container).classed('DATABASE_VIEW', true);
		 ...
		 * Add Data Info
		 ***************************************/
		// Show the data column and its dimensions 
		const dimensionDomains = this.db.dimensionDomains;
		const keys = Object.keys(dimensionDomains)
		// console.log("dimensions:",keys, dimensionDomains)
		const svg = this.header.append('svg').attr('width', 500).attr('height', 500);
		svg.append('text').attr("x", 20).attr('y', 50).classed('datainfo-text-header', true)
			.text("Variables and Ranges:")
		svg.selectAll("#colume-header")
					.data(keys)
					.enter()
					.append("text")
					.attr("x", 20)
					...
					.text((d, i) => {
						if (d !== 'FILE'){
							const curr_min = dimensionDomains[d][0];
							const curr_max = dimensionDomains[d][1];
							return d + ": [" + String(curr_min) + ", " + String(curr_max) + "]";
						}
						else
							return "Number of " + d + ": " + String(dimensionDomains[d].length)

					});			  
	}
	//establish prototype chain
	CINEMA_COMPONENTS.DatabaseView.prototype = Object.create(CINEMA_COMPONENTS.Component.prototype);
	CINEMA_COMPONENTS.DatabaseView.prototype.constructor = CINEMA_COMPONENTS.DatabaseView;
	
	CINEMA_COMPONENTS.DatabaseView.prototype.displayDatabaseSchema = function(){
		this.diagram = d3.select(this.container).append('div').attr('id', 'er_diagram')
		.style('position', 'absolute')
		.style('width', '100%');
		const svg = d3.select("#er_diagram").append("svg")
			// .attr("width", 500)
			// .attr("height", 500);	
		svg.append('text')
					// .attr('xlink:href', this.diagram_path)
				.attr('x', 0)
				.attr("y", 0)
				.text(this.db.dsi_db_schema);
				// .attr("width", 500)
				// .attr("height", 500);
	};
	/**
	 ...
	 */
	CINEMA_COMPONENTS.DatabaseView.prototype.updateSize = function() {
		//Call super
		CINEMA_COMPONENTS.Component.prototype.updateSize.call(this);
	...
	// CINEMA_COMPONENTS.DatabaseView.prototype.getDatabaseSchema = function(){
	// 	const dsi_db = this.db.dsi_db;
	// 	dsi_db.all("SELECT name FROM sqlite_master WHERE type='table'", (err, rows) => {
	// 		if (err) {
	...
	// 			db.all(`SELECT sql FROM sqlite_master WHERE name='${table}'`, (err, rows) => {
	// 			  if (err) {
	// 				console.error(err.message);
	// 			  } else {
	// 				console.log(`Schema for ${table}:`, rows[0].sql);
	// 			  }
	// 			});
	// 		  });
	// 		}
	// 	  });
	// 	// const stmt = dsi_db.prepare("");
	// 	// while(stmt.step()) { 
    //     //     const row = stmt.getAsObject();
    //     //     const temp = JSON.parse(JSON.stringify(row));
    //     //     // console.log("query", temp)
...
(function() {
	...
	function findKeysWithDifferentValues(arr) {
		...
		for (const key of keys) {
			// TODO: add halo_tag but make the text in multiple lines
			const checked = (key !== 'halo_tag' && key !== 'FSN' && key !== 'VEL' && key != 'BETA' && key !== 'SEED' && key != 'TEXP' && key != 'halo_center_x'
				&& key != 'halo_center_y' && key != 'halo_center_z' && key != 'FILE2');
			if (key !== "FILE" && checked){
	...
	CINEMA_COMPONENTS.ImageSpread.prototype.populateResults = function() {
		var self = this;
		if(this.hasFileDimensions) {
			var fileTitles = findKeysWithDifferentValues(self.db.data)
			...
			displays.enter() //add new dataDisplays
				...
				.each(function(d) {
					...
					var title = "";
					fileTitles.map(function(prefix){
						title += prefix + ": " + self.db.data[d][prefix] + "; ";
					})
					...
					// d3.select('#viewContainer').select('.dataDisplay').append('div').classed('displayLabel', true);
					// Add displayLabel div under each dataDisplay
					var displayTitles = d3.select(this)
						.selectAll('.displayLabel')
						.data([null]) // bind a single data point to ensure one element
					displayTitles.exit().remove()
					displayTitles.enter()
						.append('div')
						.classed('displayLabel', true)
						.merge(displayTitles)
						.text(title); // Customize this text as needed
					var UPDATE = ENTER.merge(fileDisplays)
						...
						.each(function(f, i) {
							...
							//Update label
							// d3.select(this).select('.displayLabel').text(title)
...
(function() {
		...
		this.axes.append('g')
			...
			.each(function(){
				...
				.on('dblclick', function() {
                    // Clear the brush on double-click
                    d3.select(this).call(self.brush.move, null);
					self.updateSQLQuerySelectionsWhenRemovingBrush();
				})
			});
	 ...
	 */
	// Edited by Mengjiao 
	CINEMA_COMPONENTS.Pcoord.prototype.updateSQLQuerySelections = function(){
		var self = this; 
		// find current selected query 
		// const current_selection = d3.select("#queries").node().value;
		if(!self.dontUpdateSelectionOnBrush){
			const current_query_text = self.db.current_selected_query;
			var new_query_text = current_query_text;
			// console.log("current selection:", , self.db.queries[current_selection]);
			const keys = Object.keys(self.brushExtents);
			keys.forEach((key) =>{
				const extent = self.brushExtents[key];
				if(extent){
					const dataRange = extent.map(self.y[key].invert);
					new_query_text += " AND " + key + " BETWEEN " + dataRange[1].toFixed(1) + " and " + dataRange[0].toFixed(1);
				}
			})
			// console.log("new query: ", new_query_text);
			// console.log("length 0", self.db.queries.length)
			self.db.queries.push(new_query_text);
			const selectElement = document.getElementById("queries");
			const newOption = document.createElement("option");
			newOption.value = String(self.db.queries.length); 
			// console.log("length 1", self.db.queries.length)
			newOption.text = new_query_text;
			selectElement.appendChild(newOption);
			selectElement.value = newOption.value;
		}

	}
	CINEMA_COMPONENTS.Pcoord.prototype.updateSQLQuerySelectionsWhenRemovingBrush = function(){
		var self = this; 
		self.db.queries = [...new Set(self.db.queries)];
		// console.log("queries", self.db.queries)
		const selectElement = document.getElementById('queries');
    	const seenOptions = new Set();
		selectElement.value = self.db.current_selected_query;
		// console.log("current query", self.db.current_selected_query)
		for (let i = selectElement.options.length - 1; i >= 0; i--) {
			const optionText = selectElement.options[i].text;
			// If the option's text is already seen, remove the option
		...
		for (let i = 0; i < selectElement.options.length; i++) {
			if (selectElement.options[i].text === self.db.current_selected_query) {
	...
	CINEMA_COMPONENTS.Pcoord.prototype.updateSelection = function(force) {
		var self = this;
		var newSelection = [];
		// console.log("self brush extents", self.brushExtents)
